...
@section('title', 'Detail Pencairan Kurir — TobaNiaga')
@section('page_title', 'Detail Pencairan Kurir')

@section('content')
<div class="max-w-2xl">

    <a href="{{ route('admin.pencairan-kurir.index') }}"
    ...
    @php
        $badge = match($pencairanDanaKurir->status) {
            ...
            default    => ['bg-gray-50 text-gray-600 border-gray-200', $pencairanDanaKurir->status],
        };
    @endphp
    ...
    <div class="bg-paper border border-lake-900/10 rounded-xl overflow-hidden mb-6">
        <div class="bg-lake-900 text-paper px-6 py-5">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="font-mono font-semibold text-base">{{ $pencairanDanaKurir->no_pencairan }}</p>
                    <p class="text-xs text-paper/50 mt-1">{{ $pencairanDanaKurir->created_at->translatedFormat('d M Y, H:i') }}</p>
                </div>
        ...
        <div class="px-6 py-5 border-b border-lake-900/8 grid grid-cols-2 gap-4">
            <div>
                <p class="text-xs text-ink/40 mb-1">Kurir</p>
                <p class="text-sm font-medium text-ink">{{ $pencairanDanaKurir->courier->nama }}</p>
                <p class="text-xs text-ink/50">{{ $pencairanDanaKurir->courier->email }}</p>
            </div>
            <div>
                <p class="text-xs text-ink/40 mb-1">Rekening Tujuan</p>
                <p class="text-sm font-medium text-ink">{{ $pencairanDanaKurir->rekeningBankKurir->nama_bank ?? '-' }}</p>
                <p class="text-xs text-ink/60">{{ $pencairanDanaKurir->rekeningBankKurir->nama_pemilik ?? '-' }}</p>
                <p class="text-xs font-mono text-ink/60">{{ $pencairanDanaKurir->rekeningBankKurir->no_rekening ?? '-' }}</p>
            </div>
        </div>

        {{-- Daftar pengiriman --}}
        <div class="px-6 py-5 border-b border-lake-900/8">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Pengiriman Termasuk ({{ $pencairanDanaKurir->detail->count() }})</p>
            <div class="space-y-2">
                @foreach($pencairanDanaKurir->detail as $d)
                <div class="flex items-center justify-between text-sm">
                    <span class="font-mono text-ink/70">{{ $d->pengiriman->pesanan->no_pesanan ?? '-' }}</span>
        ...
        <div class="px-6 py-5 bg-lake-50 flex justify-between items-center">
            ...
            <p class="text-xl font-bold text-lake-900">Rp{{ number_format($pencairanDanaKurir->jumlah, 0, ',', '.') }}</p>
        </div>

        @if($pencairanDanaKurir->diprosesOleh)
        <div class="px-6 py-4 border-t border-lake-900/8">
            <p class="text-xs text-ink/40">
                {{ $pencairanDanaKurir->status === 'ditolak' ? 'Ditolak' : 'Diproses' }} oleh
                <span class="font-medium text-ink/70">{{ $pencairanDanaKurir->diprosesOleh->nama }}</span>
                · {{ $pencairanDanaKurir->diproses_at?->format('d M Y, H:i') }}
            </p>
        </div>
        @endif

        @if($pencairanDanaKurir->catatan_admin)
        <div class="px-6 py-4 border-t border-lake-900/8">
            <p class="text-xs text-ink/40 mb-1">Catatan</p>
            <p class="text-sm text-ink/70">{{ $pencairanDanaKurir->catatan_admin }}</p>
        </div>
        @endif
    </div>

    {{-- Actions sesuai status --}}
    @if($pencairanDanaKurir->status === 'diajukan')
    <div class="flex gap-3">
        <form action="{{ route('admin.pencairan-kurir.proses', $pencairanDanaKurir) }}" method="POST" class="flex-1">
            @csrf
            <button type="submit"
    ...
    @if($pencairanDanaKurir->status === 'diproses')
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-4">
        ...
        <p class="text-xs text-blue-600">
            Transfer Rp{{ number_format($pencairanDanaKurir->jumlah, 0, ',', '.') }} ke
            {{ $pencairanDanaKurir->rekeningBankKurir->nama_bank ?? '-' }} a.n {{ $pencairanDanaKurir->rekeningBankKurir->nama_pemilik ?? '-' }}
            ({{ $pencairanDanaKurir->rekeningBankKurir->no_rekening ?? '-' }}) di luar sistem, lalu tandai selesai di bawah.
        </p>
    </div>
    <form action="{{ route('admin.pencairan-kurir.selesai', $pencairanDanaKurir) }}" method="POST" class="flex gap-3">
        @csrf
    ...
    <dialog id="modalTolak" class="rounded-xl p-0 backdrop:bg-ink/40 w-full max-w-sm">
        <form action="{{ route('admin.pencairan-kurir.tolak', $pencairanDanaKurir) }}" method="POST" class="p-5 space-y-3">
            @csrf
            <h3 class="font-display text-base font-medium text-lake-900">Tolak Pengajuan</h3>
            <textarea name="catatan_admin" rows="3" required
                      placeholder="Alasan penolakan (wajib diisi, akan dilihat kurir)"
