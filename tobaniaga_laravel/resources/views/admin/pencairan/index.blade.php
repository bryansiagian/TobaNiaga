    ...
    <div class="bg-paper border border-lake-900/10 rounded-xl overflow-hidden">
        <div class="px-5 py-4 flex items-center justify-between gap-4">
            <div class="min-w-0">
                ...
                <p class="text-sm text-ink/70 mt-1">{{ $pc->umkm->nama_umkm ?? '-' }}</p>
                <p class="text-xs text-ink/40 mt-0.5">
                    {{ $pc->created_at->format('d M Y, H:i') }} · {{ $pc->detail->count() }} pesanan
                    · {{ $pc->rekeningBank->nama_bank ?? '-' }} a.n {{ $pc->rekeningBank->nama_pemilik ?? '-' }}
                </p>
            ...
            <div class="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <a href="{{ route('admin.pencairan.show', $pc) }}"
