...
@section('title', 'Promo Platform — TobaNiaga')
@section('page_title', 'Promo Platform')

@section('content')
<div x-data="promoAdminPage()" x-init="init()">

    {{-- Header --}}
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="font-display text-xl font-medium text-lake-900">Promo Platform</h2>
            <p class="text-sm text-ink/50 mt-0.5">Promo yang berlaku lintas UMKM di TobaNiaga</p>
        </div>
    ...
    <div class="bg-paper border border-lake-900/10 rounded-xl p-16 text-center">
        ...
        <p class="text-sm text-ink/40 mb-4">Belum ada promo platform.</p>
        <button @click="openCreate()"
            ...
            <div class="px-5 py-4 flex items-start justify-between gap-4">
                <div class="flex items-start gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-ink/40 mt-0.5">
                            @if($p->umkmTarget->isEmpty())
                                Berlaku untuk: <span class="text-lake-800 font-medium">Semua UMKM</span>
                            @else
                                Berlaku untuk: <span class="text-lake-800 font-medium">{{ $p->umkmTarget->pluck('nama_umkm')->join(', ') }}</span>
                            @endif
                        </p>
                    </div>
                </div>
                <div class="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="openEdit({{ $p->toJson() }}, {{ $p->umkmTarget->pluck('id')->toJson() }})"
                        ...
                        <form action="{{ route('admin.promo.destroy', $p) }}" method="POST"
    ...
    <div x-show="modalOpen" x-cloak
        ...
        <div class="relative w-full sm:max-w-lg bg-paper rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[92vh] overflow-y-auto"
            ...
            <div class="flex items-center justify-between px-5 pt-4 pb-3 border-b border-lake-900/8">
                <h3 class="font-display text-lg font-medium text-lake-900" x-text="isEdit ? 'Edit Promo' : 'Buat Promo Platform'"></h3>
            ...
            <form :action="isEdit ? editUrl : '{{ route('admin.promo.store') }}'"
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="form.nama_promo"
                               placeholder="Harbolnas TobaNiaga"
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="number" x-model="form.nilai" min="1" step="any"
                               class="w-full border border-lake-900/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lake-900/20">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" x-model="form.min_belanja" min="0" placeholder="0"
                               class="w-full border border-lake-900/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lake-900/20">
                    </div>
                    <div x-show="form.tipe === 'persen'">
                        <label class="block text-xs font-medium text-lake-900 mb-1">Maks. Potongan (Rp)</label>
                        <input type="number" x-model="form.maks_diskon" min="0" placeholder="Tidak terbatas"
                               class="w-full border border-lake-900/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lake-900/20">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="date" x-model="form.berlaku_mulai"
                               class="w-full border border-lake-900/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lake-900/20">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-lake-900 mb-1">Berlaku Sampai</label>
                        <input type="date" x-model="form.berlaku_sampai"
                               class="w-full border border-lake-900/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lake-900/20">
                    </div>
                </div>
                ...
                {{-- Kuota --}}
                <div>
                    <label class="block text-xs font-medium text-lake-900 mb-1">Kuota <span class="text-ink/40 font-normal">(kosong = tidak terbatas)</span></label>
                    <input type="number" x-model="form.kuota" min="1" placeholder="Tidak terbatas"
                           class="w-full border border-lake-900/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lake-900/20">
                </div>
                ...
                <div>
                    <label class="block text-xs font-medium text-lake-900 mb-1">
                        Target UMKM <span class="text-ink/40 font-normal">(kosong = semua UMKM)</span>
                    </label>
                    <div class="border border-lake-900/15 rounded-lg divide-y divide-lake-900/8 max-h-40 overflow-y-auto">
                        @foreach($umkmList as $u)
                        <label class="flex items-center gap-2.5 px-3 py-2.5 cursor-pointer hover:bg-lake-50">
                            <input type="checkbox" name="umkm_ids[]" value="{{ $u->id }}"
                                   :checked="form.umkm_ids.includes({{ $u->id }})"
                                   @change="toggleUmkm({{ $u->id }})"
                                   class="w-4 h-4 accent-lake-800 rounded flex-shrink-0">
                            <span class="text-sm text-ink/80 flex-1 min-w-0 truncate">{{ $u->nama_umkm }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
...
function promoAdminPage() {
    return {
        ...
        init() {
            @if($errors->any())
                this.form = {
                    ...
                    umkm_ids:       {!! json_encode(old('umkm_ids', [])) !!},
                    is_aktif:       true,
                };
                this.modalOpen = true;
            @endif
        ...
        openEdit(promo, umkmIds) {
            this.isEdit = true;
            this.editUrl = `/admin/promo/${promo.id}`;
            this.form = {
                kode:           promo.kode,
        ...
        toggleUmkm(id) {
            const idx = this.form.umkm_ids.indexOf(id);
            if (idx === -1) this.form.umkm_ids.push(id);
            else this.form.umkm_ids.splice(idx, 1);
        },
    }
}
</script>
