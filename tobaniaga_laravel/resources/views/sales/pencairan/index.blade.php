...
<div x-data="pencairanPage()" x-init="init()">
    ...
    <div class="grid sm:grid-cols-3 gap-4 mb-8">
        <div class="stat-card">
            ...
            <p class="text-xs text-ink/50 mt-1">{{ $pesananEligible->count() }} pesanan siap dicairkan</p>
        </div>
        <div class="stat-card">
    ...
    <div class="mb-6 px-5 py-4 rounded-xl bg-yellow-50 border border-yellow-200 flex items-start gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-yellow-600 mt-0.5">
                ...
                <a href="{{ route('sales.rekening.index') }}" class="underline font-medium">Rekening Bank</a>
    ...
    <div class="bg-paper border border-lake-900/10 rounded-xl overflow-hidden mb-8">
        ...
        <div class="px-6 py-14 text-center">
            ...
            <p class="text-sm text-ink/40">Belum ada pesanan yang siap dicairkan.</p>
            <p class="text-xs text-ink/30 mt-1">Pesanan akan muncul di sini setelah berstatus "Selesai".</p>
        </div>
        @else
        <form action="{{ route('sales.pencairan.store') }}" method="POST">
            @csrf
            ...
            <div class="px-6 py-4 border-b border-lake-900/8">
                ...
                <div class="space-y-2">
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors"
                        ...
                        <input type="radio" name="rekening_bank_id" value="{{ $r->id }}"
            ...
            <div class="divide-y divide-lake-900/6 max-h-96 overflow-y-auto">
                @foreach($pesananEligible as $p)
                <label class="flex items-center justify-between gap-4 px-6 py-3.5 cursor-pointer hover:bg-lake-50/50">
                    <div class="flex items-center gap-3 min-w-0">
                        <input type="checkbox" name="pesanan_ids[]" value="{{ $p->id }}"
                               x-model="terpilih"
                               class="w-4 h-4 accent-lake-800 rounded flex-shrink-0">
                        <div class="min-w-0">
                            <p class="text-sm font-mono font-medium text-ink truncate">{{ $p->no_pesanan }}</p>
                            <p class="text-xs text-ink/40">{{ $p->created_at->format('d M Y') }} · {{ $p->detail->count() }} produk</p>
                        </div>
                    </div>
                    <p class="text-sm font-semibold text-lake-900 flex-shrink-0">
                        Rp{{ number_format($p->total_harga - $p->ongkos_kirim, 0, ',', '.') }}
                    </p>
                </label>
                @endforeach
            </div>
            ...
            <div class="px-6 py-4 flex items-center justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-ink/40 mt-0.5">
                        {{ $pc->created_at->format('d M Y, H:i') }} · {{ $pc->detail->count() }} pesanan · {{ $pc->rekeningBank->nama_bank }}
                    </p>
...
function pencairanPage() {
    return {
        ...
        semuaTerpilih: false,
        pesananData: {!! $pesananEligible->mapWithKeys(fn($p) => [$p->id => $p->total_harga - $p->ongkos_kirim])->toJson() !!},

        init() {},

        get totalTerpilih() {
            return this.terpilih.reduce((sum, id) => sum + (this.pesananData[id] || 0), 0);
        },
        ...
        pilihSemua() {
            if (this.semuaTerpilih) {
                ...
                this.terpilih = Object.keys(this.pesananData).map(Number);
            }
