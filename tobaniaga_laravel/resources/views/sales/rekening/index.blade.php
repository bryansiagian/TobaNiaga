...
<div x-data="rekeningPage()" x-init="init()">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-ink/50 mt-0.5">Rekening tujuan pencairan dana tokomu</p>
        </div>
        ...
        <div class="bg-paper border rounded-xl overflow-hidden {{ $r->is_utama ? 'border-lake-800/30' : 'border-lake-900/10' }}">
            <div class="px-5 py-4 flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3 flex-shrink-0">
                    @if(!$r->is_utama)
                    <form action="{{ route('sales.rekening.utama', $r) }}" method="POST">
                        @csrf @method('PATCH')
                    ...
                    <form action="{{ route('sales.rekening.destroy', $r) }}" method="POST"
    ...
    <div x-show="modalOpen" x-cloak
        ...
        <div class="relative w-full sm:max-w-md bg-paper rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[92vh] overflow-y-auto"
            ...
            <form :action="isEdit ? editUrl : '{{ route('sales.rekening.store') }}'"
...
function rekeningPage() {
    return {
        ...
        openEdit(rekening) {
            this.isEdit = true;
            this.editUrl = `/sales/rekening/${rekening.id}`;
            this.form = {
