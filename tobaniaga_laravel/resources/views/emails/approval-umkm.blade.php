...
<title>Update UMKM TobaNiaga</title>
<style>
  ...
  .badge-approved { display: inline-block; background: #1a4a6b15; color: #1a4a6b; border: 1px solid #1a4a6b30; border-radius: 6px; padding: 4px 12px; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 20px; }
  .badge-rejected { display: inline-block; background: #8f233310; color: #8f2333; border: 1px solid #8f233330; border-radius: 6px; padding: 4px 12px; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 20px; }
...
<div class="wrap">
  ...
  <div class="body">
    ...
    @if($status === 'approved')
      <div class="badge-approved">✓ Disetujui</div>
      <h2>Selamat, UMKM kamu telah diverifikasi!</h2>
      <p>Halo <strong>{{ $umkm->owner->nama }}</strong>,</p>
      <p>
        UMKM <strong>"{{ $umkm->nama_umkm }}"</strong> kamu telah berhasil diverifikasi oleh tim TobaNiaga.
        Kamu sekarang bisa login dan mulai menambahkan produk untuk dijual.
      </p>
      <a href="{{ route('login') }}" class="cta">Login ke Dashboard</a>

    @else
      <div class="badge-rejected">✗ Tidak Disetujui</div>
      <h2>Update pendaftaran UMKM kamu</h2>
      <p>Halo <strong>{{ $umkm->owner->nama }}</strong>,</p>
      <p>
        Setelah ditinjau, pendaftaran UMKM <strong>"{{ $umkm->nama_umkm }}"</strong>
